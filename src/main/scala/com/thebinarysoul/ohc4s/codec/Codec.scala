...
import com.thebinarysoul.ohc4s.codec.Sizes.*
...
given Codec[Byte] with
  override def encode(value: Byte): Buffer = encodeWith(ONE)(_.put(value))
  override def decode(buffer: Buffer): Byte = buffer.get

given Codec[Short] with
  override def encode(value: Short): Buffer = encodeWith(TWO)(_.putShort(value))
  override def decode(buffer: Buffer): Short = buffer.getShort

given Codec[Int] with
  override def encode(value: Int): Buffer = encodeWith(FOUR)(_.putInt(value))
  override def decode(buffer: Buffer): Int = buffer.getInt

given Codec[Long] with
  override def encode(value: Long): Buffer = encodeWith(EIGHT)(_.putLong(value))
  override def decode(buffer: Buffer): Long = buffer.getLong

given Codec[Float] with
  override def encode(value: Float): Buffer = encodeWith(FOUR)(_.putFloat(value))
  override def decode(buffer: Buffer): Float = buffer.getFloat

given Codec[Double] with
  override def encode(value: Double): Buffer = encodeWith(EIGHT)(_.putDouble(value))
  override def decode(buffer: Buffer): Double = buffer.getDouble

given Codec[Boolean] with
  override def encode(value: Boolean): Buffer = encodeWith(ONE)(_.put(if value then ONE else ZERO))
  override def decode(buffer: Buffer): Boolean = buffer.get == ONE

given Codec[String] with
  override def encode(value: String): Buffer =
    val bytes = value.getBytes
    merge(
      encodeWith(FOUR)(_.putInt(bytes.length)),
...
implicit inline def optCodec[T](using codec: Codec[T]): Codec[Option[T]] = new Codec[Option[T]] :
  override def encode(value: Option[T]): Buffer = value match {
    case Some(value) =>
      merge(
        encodeWith(ONE)(_.put(ONE)),
        codec.encode(value)
      )
    case _ => encodeWith(ONE)(_.put(ZERO))
  }
...
implicit inline def seqCodec[T](using codec: Codec[T]): Codec[List[T]] = new Codec[List[T]] :
  override def encode(value: List[T]): Buffer =
    val sizeBuffer = encodeWith(FOUR)(_.putInt(value.size))
    value
      .map(codec.encode)
